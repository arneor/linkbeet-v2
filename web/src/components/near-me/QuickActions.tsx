...
export const QUICK_ACTIONS = [
  {
    label: 'Top Rated',
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-[22px] h-[22px] text-slate-700">
    ...
    <div className="flex items-center justify-between gap-2 mb-8 mt-2 px-1">
      {QUICK_ACTIONS.map((a) => (
        ...
        >
          ...
          <span className="text-[11px] font-medium text-slate-600 whitespace-nowrap text-center">{a.label}</span>
        </button>
      ))}
    </div>
  )
